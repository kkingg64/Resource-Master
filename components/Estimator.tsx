        ...
        <table className="w-full text-left border-collapse table-fixed">
            ...
            <tbody className="text-[11px]">
                ...
                {groupedTotals.map(([type, subtotal]) => {
                    const modulesOfType = modules.filter(m => (m.type || ModuleType.Development) === type);
                    if (modulesOfType.length === 0) return null;

                    return (
                        <React.Fragment key={type}>
                            {modulesOfType.map(m => {
                                const index = modules.findIndex(mod => mod.id === m.id);
                                const moduleType = m.type || ModuleType.Development;
                                const isDev = moduleType === ModuleType.Development;
                                ...
                                return (
                                    ...
                                    <tr draggable={!isReadOnly} onDragStart={(e) => handleDragStart(e, index)} onDragOver={handleDragOver} onDrop={(e) => handleDrop(e, index)} className={`hover:bg-slate-50 transition-colors group ${draggedIndex === index ? 'opacity-40' : ''} ${typeStyle.rowBg}`} onContextMenu={(e) => { if (isReadOnly) return; e.preventDefault(); e.stopPropagation(); setContextMenu({ x: e.pageX, y: e.pageY, moduleId: m.id }); }}>
                                        ...
                                        <td className="p-0 border-b border-slate-100"><EstimatorNumberInput rowIndex={index * 1000} colIndex={1} onNavigate={handleNavigate} className={`${baseInputClass} text-slate-500`} min={1} value={m.prepVelocity} onChange={(val) => updateEstimates(val, m.prepTeamSize || 2, m.frontendVelocity || 5, m.frontendTeamSize || 2, m.backendVelocity || 5, m.backendTeamSize || 2)} disabled={isReadOnly || isDev} /></td>
                                        <td className="p-0 border-b border-slate-100"><EstimatorNumberInput rowIndex={index * 1000} colIndex={2} onNavigate={handleNavigate} className={`${baseInputClass} text-slate-500`} min={1} value={m.prepTeamSize} onChange={(val) => updateEstimates(m.prepVelocity || 10, val, m.frontendVelocity || 5, m.frontendTeamSize || 2, m.backendVelocity || 5, m.backendTeamSize || 2)} disabled={isReadOnly || isDev} /></td>
                                        <td className="p-0 border-b border-slate-100"><ComplexitySelect value={m.complexity || 'Medium'} onChange={(val) => onUpdateModuleComplexity(selectedProjectId, m.id, 'prep', val)} isReadOnly={isReadOnly || isDev} /></td>
                                        <td className="px-1 text-center border-b border-slate-100 text-slate-400 font-mono">{modulePrepEffort || '-'}</td>
                                        ...
                                        <td className="p-0 border-b border-slate-100"><EstimatorNumberInput rowIndex={index * 1000} colIndex={4} onNavigate={handleNavigate} className={`${baseInputClass} text-slate-500`} min={1} value={m.frontendVelocity} onChange={(val) => updateEstimates(m.prepVelocity || 10, m.prepTeamSize || 2, val, m.frontendTeamSize || 2, m.backendVelocity || 5, m.backendTeamSize || 2)} disabled={isReadOnly || !isDev} /></td>
                                        <td className="p-0 border-b border-slate-100"><EstimatorNumberInput rowIndex={index * 1000} colIndex={5} onNavigate={handleNavigate} className={`${baseInputClass} text-slate-500`} min={1} value={m.frontendTeamSize} onChange={(val) => updateEstimates(m.prepVelocity || 10, m.prepTeamSize || 2, m.frontendVelocity || 5, val, m.backendVelocity || 5, m.backendTeamSize || 2)} disabled={isReadOnly || !isDev} /></td>
                                        ...
                                        <td className="p-0 border-b border-slate-100"><EstimatorNumberInput rowIndex={index * 1000} colIndex={7} onNavigate={handleNavigate} className={`${baseInputClass} text-slate-500`} min={1} value={m.backendVelocity} onChange={(val) => updateEstimates(m.prepVelocity || 10, m.prepTeamSize || 2, m.frontendVelocity || 5, m.frontendTeamSize || 2, val, m.backendTeamSize || 2)} disabled={isReadOnly || !isDev} /></td>
                                        <td className="p-0 border-b border-slate-100"><EstimatorNumberInput rowIndex={index * 1000} colIndex={8} onNavigate={handleNavigate} className={`${baseInputClass} text-slate-500`} min={1} value={m.backendTeamSize} onChange={(val) => updateEstimates(m.prepVelocity || 10, m.prepTeamSize || 2, m.frontendVelocity || 5, m.frontendTeamSize || 2, m.backendVelocity || 5, val)} disabled={isReadOnly || !isDev} /></td>
