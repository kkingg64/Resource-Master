    ...
    modules.forEach(m => {
        ...
        if (isDev) {
            m.tasks.forEach(t => {
                ...
                const taskFeTeam = t.frontendTeamSize ?? m.frontendTeamSize ?? 2;
                ...
                const taskBeTeam = t.backendTeamSize ?? m.backendTeamSize ?? 2;
      ...
      <div className="flex-1 overflow-auto bg-white relative">
        <table className="w-full text-left border-collapse table-fixed">
            ...
            <tbody className="text-[11px]">
                {modules.length === 0 && (
                    <tr><td colSpan={22} className="p-12 text-center text-slate-400 bg-slate-50/30"><div className="flex flex-col items-center justify-center gap-2"><Layers className="w-8 h-8 text-slate-300"/><p>No modules found.</p></div></td></tr>
                )}
                {modules.map((m, index) => {
                    const moduleType = m.type || ModuleType.Development;
                    const isDev = moduleType === ModuleType.Development;
                    ...
                    if (isDev) { m.tasks.forEach(t => { const taskFeVel = t.frontendVelocity ?? m.frontendVelocity ?? 5; const taskFeComp = t.frontendComplexity ?? m.frontendComplexity ?? 'Medium'; moduleFeEffort += (t.frontendFunctionPoints ?? 0) > 0 ? Math.ceil(((t.frontendFunctionPoints ?? 0) / taskFeVel) * COMPLEXITY_MULTIPLIERS[taskFeComp]) : 0; const taskBeVel = t.backendVelocity ?? m.backendVelocity ?? 5; const taskBeComp = t.backendComplexity ?? m.backendComplexity ?? 'Medium'; moduleBeEffort += (t.backendFunctionPoints ?? 0) > 0 ? Math.ceil(((t.backendFunctionPoints ?? 0) / taskBeVel) * COMPLEXITY_MULTIPLIERS[taskBeComp]) : 0; }); }
                    ...
                    return (
                        ...
                        <tr draggable={!isReadOnly} onDragStart={(e) => handleDragStart(e, index)} onDragOver={handleDragOver} onDrop={(e) => handleDrop(e, index)} className={`hover:bg-slate-50 transition-colors group ${draggedIndex === index ? 'opacity-40' : ''} ${typeStyle.rowBg}`}>
                            ...
                            <td className="px-2 py-1.5 border-b border-slate-100 border-r border-slate-200"><div className="flex items-center gap-1.5">{isDev && <button onClick={() => toggleModuleExpansion(m.id)} className="p-0.5 rounded hover:bg-black/5">{isExpanded ? <ChevronDown size={14}/> : <ChevronRight size={14}/>}</button>}<Icon size={14} className="text-slate-400" title={MODULE_TYPE_DISPLAY_NAMES[moduleType]} /><span className="font-medium text-slate-700 truncate block w-full" title={m.name}>{m.name}</span></div></td>
                            <td className="p-0 border-b border-slate-100 relative hover:bg-white/50"><EstimatorNumberInput rowIndex={index * 1000} colIndex={0} onNavigate={handleNavigate} className={`${baseInputClass} text-slate-600`} value={m.legacyFunctionPoints || 0} onChange={(val) => updateEstimates(val, prepVelocity, prepTeamSize, m.frontendVelocity || 5, m.frontendTeamSize || 2, m.backendVelocity || 5, m.backendTeamSize || 2)} disabled={isReadOnly} /></td>
                            <td className="p-0 border-b border-slate-100 relative hover:bg-white/50"><EstimatorNumberInput rowIndex={index * 1000} colIndex={1} onNavigate={handleNavigate} className={`${baseInputClass} text-slate-500`} min={1} value={prepVelocity} onChange={(val) => updateEstimates(m.legacyFunctionPoints || 0, val, prepTeamSize, m.frontendVelocity || 5, m.frontendTeamSize || 2, m.backendVelocity || 5, m.backendTeamSize || 2)} disabled={isReadOnly} /></td>
                            <td className="p-0 border-b border-slate-100 relative hover:bg-white/50"><EstimatorNumberInput rowIndex={index * 1000} colIndex={2} onNavigate={handleNavigate} className={`${baseInputClass} text-slate-500`} min={1} value={prepTeamSize} onChange={(val) => updateEstimates(m.legacyFunctionPoints || 0, prepVelocity, val, m.frontendVelocity || 5, m.frontendTeamSize || 2, m.backendVelocity || 5, m.backendTeamSize || 2)} disabled={isReadOnly} /></td>
                            ...
                            <td className="p-0 border-b border-slate-100 relative"><EstimatorNumberInput rowIndex={index * 1000} colIndex={3} onNavigate={handleNavigate} className={`${baseInputClass} font-bold text-slate-800`} value={moduleFeFp} onChange={() => {}} disabled={true} /></td>
                            <td className="p-0 border-b border-slate-100 relative hover:bg-white/50"><EstimatorNumberInput rowIndex={index * 1000} colIndex={4} onNavigate={handleNavigate} className={`${baseInputClass} text-slate-500`} min={1} value={m.frontendVelocity} onChange={(val) => updateEstimates(m.legacyFunctionPoints || 0, prepVelocity, prepTeamSize, val, feTeam, m.backendVelocity || 5, m.backendTeamSize || 2)} disabled={isReadOnly || !isDev} /></td>
                            <td className="p-0 border-b border-slate-100 relative hover:bg-white/50"><EstimatorNumberInput rowIndex={index * 1000} colIndex={5} onNavigate={handleNavigate} className={`${baseInputClass} text-slate-500`} min={1} value={feTeam} onChange={(val) => updateEstimates(m.legacyFunctionPoints || 0, prepVelocity, prepTeamSize, m.frontendVelocity || 5, val, m.backendVelocity || 5, m.backendTeamSize || 2)} disabled={isReadOnly || !isDev} /></td>
                            <td className="p-0 border-b border-slate-100"><ComplexitySelect value={m.frontendComplexity || 'Medium'} onChange={(val) => onUpdateModuleComplexity(selectedProjectId, m.id, 'frontend', val)} isReadOnly={isReadOnly || !isDev} /></td>
                            <td className="px-1 text-center border-b border-slate-100 text-slate-400 font-mono">{moduleFeEffort || '-'}</td>
                            ...
                            <td className="p-0 border-b border-slate-100 relative"><EstimatorNumberInput rowIndex={index * 1000} colIndex={6} onNavigate={handleNavigate} className={`${baseInputClass} font-bold text-slate-800`} value={moduleBeFp} onChange={() => {}} disabled={true} /></td>
                            <td className="p-0 border-b border-slate-100 relative hover:bg-white/50"><EstimatorNumberInput rowIndex={index * 1000} colIndex={7} onNavigate={handleNavigate} className={`${baseInputClass} text-slate-500`} min={1} value={m.backendVelocity} onChange={(val) => updateEstimates(m.legacyFunctionPoints || 0, prepVelocity, prepTeamSize, m.frontendVelocity || 5, m.frontendTeamSize || 2, val, beTeam)} disabled={isReadOnly || !isDev} /></td>
                            <td className="p-0 border-b border-slate-100 relative hover:bg-white/50"><EstimatorNumberInput rowIndex={index * 1000} colIndex={8} onNavigate={handleNavigate} className={`${baseInputClass} text-slate-500`} min={1} value={beTeam} onChange={(val) => updateEstimates(m.legacyFunctionPoints || 0, prepVelocity, prepTeamSize, m.frontendVelocity || 5, m.frontendTeamSize || 2, m.backendVelocity || 5, val)} disabled={isReadOnly || !isDev} /></td>
                            <td className="p-0 border-b border-slate-100"><ComplexitySelect value={m.backendComplexity || 'Medium'} onChange={(val) => onUpdateModuleComplexity(selectedProjectId, m.id, 'backend', val)} isReadOnly={isReadOnly || !isDev} /></td>
                            <td className="px-1 text-center border-b border-slate-100 text-slate-400 font-mono">{moduleBeEffort || '-'}</td>
                        ...
                        {isExpanded && isDev && m.tasks.map((task, taskIndex) => {
                            ...
                            <tr key={task.id} className="bg-slate-50/50 hover:bg-slate-100/50">
                                ...
                                <td className="pl-12 pr-2 py-1 text-xs text-slate-600 border-b border-slate-100 border-r border-slate-200"><div className="flex items-center gap-2"><Dot size={16} className="text-slate-400"/> <span className="truncate" title={task.name}>{task.name}</span></div></td>
                                ...
                                <td className="p-1 border-b border-slate-100 text-center text-xs text-slate-500">{taskStartDate ? new Date(taskStartDate).toLocaleDateString(undefined, {month:'short', day:'numeric'}) : '-'}</td>
