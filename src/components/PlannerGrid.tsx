...
// ... (DatePicker and SaveStatusIndicator components remain unchanged) ...
// ... (GridNumberInput component remains unchanged) ...
// ... (ROLE_STYLES and MODULE_TYPE_STYLES remain unchanged) ...
...
export const PlannerGrid: React.FC<PlannerGridProps> = ({ 
  ...
  const timeline: TimelineColumn[] = useMemo(() => getTimeline(viewMode, timelineStart, timelineEnd), [viewMode, timelineStart, timelineEnd]);
  ...
  const assignmentRenderInfo = useMemo<{ map: Map<string, { rowIndex: number; y: number; startDate: string | undefined; endDate: string }>; totalHeight: number }>(() => {
                          ...
                          return (
                            <React.Fragment key={task.id}>
                              ...
                              {!isTaskCollapsed && task.assignments.map((assignment, assignmentIndex) => {
                                ...
                                // Calculate real-time progress based on date
                                  ...
                                  <div className="flex relative">
                                    {displayMode === 'gantt' && hasSchedule && startIndex > -1 && endIndex > -1 && (
                                        <div
                                            ...
                                            title={`${assignment.role} - ${assignment.resourceName || 'Unassigned'} (${calculatedProgress}%)`}
                                        >
                                            <div className={`h-full opacity-30 ${roleStyle.fill}`} style={{ width: `${calculatedProgress}%` }}></div>
                                            ...
                                            {calculatedProgress > 0 && (
                                                <span className="absolute right-2 text-[8px] font-bold text-slate-600 z-20 pointer-events-none bg-white/50 px-0.5 rounded">{calculatedProgress}%</span>
                                            )}
                                        </div>
                                    )}
